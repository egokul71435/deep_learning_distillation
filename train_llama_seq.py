"""Sequence-level knowledge distillation from a Llama teacher using LoRA.

Same approach as train_gpt_seq.py but reads from the llama_teacher_data/
folder instead. Only LoRA adapter weights are updated (less than 1% of
total parameters).

Usage:
    python train_llama_seq.py --dataset scibench
...
def parse_args():
    ...
    p.add_argument("--output-dir", default="./output/llama_seq_kd")
...
def main():
    ...
    print("Starting Llama sequence-KD training …")
    trainer.train()
