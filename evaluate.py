"""Evaluate distilled (or base) student models on SciBench and TheoremQA test sets.

Generates a chain-of-thought response for each test problem, extracts the final
numeric answer, and compares against ground truth with a configurable tolerance.
Also computes BERTScore between the student's CoT and the teacher's reference CoT.

Produces two output files per run:
  1. responses.jsonl: one record per test problem, written incrementally as each
     problem completes (safe against crashes).
  2. metrics.json: accuracy and BERTScore statistics per dataset and overall.

Usage:
    # Distilled model (with LoRA adapter):
    python evaluate.py \
...
def parse_args():
    ...
    p.add_argument("--adapter-path", default=None,
                    help="Path to LoRA adapter dir (omit to evaluate the base model)")
    ...
    p.add_argument("--dataset", nargs="+", default=["scibench", "theoremqa"],
                    choices=["scibench", "theoremqa"],
...
def evaluate_dataset(model, tokenizer, test_data, device, max_new_tokens,
                     tolerance, responses_file):
    """Generate + score each problem, appending to *responses_file* incrementally."""
    records = []
    student_cots = []
    teacher_cots = []
    ...
    for i, row in enumerate(test_data):
        ...
        responses_file.write(json.dumps(rec) + "\n")
...
def main():
    ...
    if args.adapter_path:
        ...
        print("No adapter specified — evaluating base model.")
        model = base_model
    model.eval()
    ...
    with open(responses_path, "w") as resp_f:
        for ds_name, test_data in datasets.items():
            ...
            records, s_cots, t_cots = evaluate_dataset(
                ...
                args.max_new_tokens, args.tolerance, resp_f,
            )
    ...
    # ---- BERTScores (separate pass — responses already on disk) -----------
    for ds_name in dataset_metrics:
        n = dataset_metrics[ds_name]["total"]
        offset = sum(dataset_metrics[d]["total"] for d in dataset_metrics if d < ds_name)
        s_cots = all_student_cots[offset:offset + n]
        t_cots = all_teacher_cots[offset:offset + n]

        print(f"  Computing BERTScores for {ds_name} …")
        bs = compute_bert_scores(s_cots, t_cots, device)
        dataset_metrics[ds_name]["bert_score"] = {
            k: v for k, v in bs.items() if k != "per_item"
        }
        dataset_metrics[ds_name]["bert_score_per_item"] = bs["per_item"]
    ...
    print(f"  Computing overall BERTScores …")
